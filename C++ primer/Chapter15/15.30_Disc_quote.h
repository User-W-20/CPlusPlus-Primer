...
//
//	Disc_quote(const std::string&b,double p,std::size_t q,double disc):Quote(b,p),quantity(q),discount(disc)
//	{
//		std::cout << "Disc_quote : constructor taking 4 parameters.\n";
//	}
//
//	Disc_quote(const Disc_quote&dq):quantity(dq.quantity),discount(dq.discount)
//	{
//		std::cout << "Disc_quote : copy constructor.\n";
//	}
//
//	Disc_quote(Disc_quote&&dq)noexcept:quantity(std::move(dq.quantity)),discount(std::move(dq.discount))
//	{
...
//{
//	return static_cast<const Quote&>(lhs) != static_cast<const Quote&>(rhs)
